...
def validate_inputs(argv):
    ...
    for col in numeric_cols.columns:
        if not pd.to_numeric(numeric_cols[col], errors="coerce").notnull().all():
            print(
                f'Error: Column "{col}" contains non-numeric values. '
                "From 2nd to last columns must contain numeric values only."
            )
            sys.exit(1)
    ...
    for impact in impacts:
        if impact not in ("+", "-"):
            print('Error: Impacts must be either +ve or -ve (use "+" or "-").')
            sys.exit(1)
    ...
    if len(weights) != num_criteria:
        print(
            f"Error: Number of weights ({len(weights)}) must match the number of "
    ...
    if len(impacts) != num_criteria:
        print(
            f"Error: Number of impacts ({len(impacts)}) must match the number of "
            f"criteria columns ({num_criteria})."
        )
        sys.exit(1)
